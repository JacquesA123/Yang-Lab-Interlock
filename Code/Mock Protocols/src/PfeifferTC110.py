...
class TC110:
    def __init__(self, device_id=1, port=None, autoconnect=True): # Creates the class
        ...
        if port == None: # If you don't assign a specific port, it defaults to using the first port it detects
    ...
    def get_status(self, device_id=None):
        ...
        rpm_speed = self.get_rpm_speed(device_id=device_id)
        pressure = self.get_pressure(device_id=device_id)
        status = {'Running':running, 'Speed':speed, 'Rpm_speed': rpm_speed, 'Pressure':pressure}
        return status
